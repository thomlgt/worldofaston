# World Of Aston : Wrath of C#


Le but de cette exercice est de developper un petit jeu de role.
 
 Pensez a gitter ce petit projet! En le documentant vous aurez tout ce q
 u'il faut savoir en C#!
 ___
...
```C#
/**
* Créer un personnage avec toutes ses attributs. Demande a l'utilisateur d'entrer le nom du personnage.
* retour: une instance de la classe Personnage correctement instancié.
**/
public static PersonnageFactory(){
    // Demender a l'utilisateur un nom de personnage
    // Creer un nouveau personnage en utilisant le constructeur avec tout ses params (dont le nom qui vient d'etre choisie par l'utilisateur)
...
1.3 Surcharger la methode ***public string ToString()*** personnage sous le format : "nom:[this.nom], pointDeVie:[this.pointDeVie], attaque:[this.attaque]"

__Remarque :__ vous pouvez utiliser le formatage de chaine avec 
```C#
String.Format("nom:{0}, points de vie:{1}, degats:{2}", this.nom, this.pointDeVie, this.degats)
```
ou
```C#
$"nom:{this.nom}, points de vie:{this.pointDeVie}, degats:{this.degats}"
```
___
## Exercice 2 : Creation du monde!
 
2.1: Créer une classe static Monde. 

Cette classe représentera le monde de notre jeu. Elle n'a pas vocation à être instancié et doit etre accéssible depuis n'importe qu'elles classes.
...
```
 ...
 PersonnageFactory doit etre accessible depuis n'importe qu'elles classes. Elle fera partie des nombreuses Factory.
...
___
 ...
 3.1: Creer une classe Monstre héritant de la classe Personnage
...
3.2: Creer une class abstraite Combattant
```C#
...
4.1.1: Ajouter a la classe ***Monde*** la methode:
```C#
...
4.1.2: Pour creer le nom de votre monstre:

Creer dans classe Monde
```C#
...
```C#
string nom = this.debutNom[new Random().Next(this.debutNom.Lenght)] + idem pour les fin de mots
```
...
4.2.2: Dans cette methode faire jouer a tour de roles les deux enemies:
Chaqu'un son tour, le Combattant enleve son nombre d'attaque a l'adversaire.
Il affiche dans la console son nom, ainsi que ces degats.
Le combat s'arrete quand l'un des deux Combattant a moins de 0 pointDeVie et
affiche la victoire a son adversaire.
```C#
...
```
 ...
 5.2: Implemeneter l'interface dans la classe AbstractCombattant.
 ```C#
abstract class AbstractCombattant : ICombattant
{
    ...
    public override void Defence(int degats)
    {
...
```
 ...
 6.2: Creer une classe BasicAttaque
 ```C#
class BasicAttaque : IAttaque
{
    ...
    {
        // Avec un nombre aleatoire rand comprie entre 0 et 100
...
```
 ...
 7.2: Creer une methode GetAttaque() qui renvois une attaque de la liste des attaques choisie aleatoirement
 ...
 Changer aussi la methode PersonnageFactory() pour definir un classe.
 
 7.5: Changer la methode Attaquer pour utiliser le GetAttaque() de la classe et passer le resultat de la methode LancerAttaque de l'Attaque dans les arguments de la methode defence.

 7.6: Netoyer votre code en supprimant les attributs inutils.
 
 ___
 Exercice 8: Choisir une classe
 
 8.1: Creer un attribut static  dictionnaire<string,string> classes dans la classe Monde. Creer une methode GetClasses(string)
 ```C#
...
```
___
8.2: Changer la methode pour PersonnageFactory pour demander la classe du personnage grace a la methode GetClasse et un Console.Read

___
 Exercice 9: Les groupes
 
 9.1: Creer une classe Groupe implementant ICombattant ayant en attribut une liste de ICombattant
...
___
 ...
 Bravo, vous avez terminé la formation C#
